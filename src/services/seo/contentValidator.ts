import { SEOCheck, SEOHeading, HeadingIssue } from '../../types/seo'
 ...
 */
export function validateH1(headings: any[]): SEOCheck[] {
    const checks: SEOCheck[] = []
...
function detectHeadingJumps(headings: SEOHeading[]): HeadingIssue[] {
    ...
    for (let i = 1; i < headings.length; i++) {
        const prevLevel = parseInt(headings[i - 1].level.charAt(1))
        const currentLevel = parseInt(headings[i].level.charAt(1))
        
        if (currentLevel > prevLevel + 1) {
            issues.push({
                type: 'jump',
                level: headings[i].level,
                index: headings[i].index,
                text: headings[i].text,
                previousHeading: {
                    level: headings[i - 1].level,
                    text: headings[i - 1].text,
                    index: headings[i - 1].index
                },
                severity: 'warning'
            })
        }
...
function detectMissingLevels(headings: SEOHeading[]): HeadingIssue[] {
    ...
    if (hasLevels.h3 && !hasLevels.h2) {
        headings.filter(h => h.level === 'h3').forEach(h3 => {
            issues.push({
                type: 'missing_level',
                level: 'h3',
                index: h3.index,
    ...
    if (hasLevels.h4 && !hasLevels.h3) {
        headings.filter(h => h.level === 'h4').forEach(h4 => {
            issues.push({
                type: 'missing_level',
                level: 'h4',
                index: h4.index,
    ...
    if (hasLevels.h5 && !hasLevels.h4) {
        headings.filter(h => h.level === 'h5').forEach(h5 => {
            issues.push({
                type: 'missing_level',
                level: 'h5',
                index: h5.index,
    ...
    if (hasLevels.h6 && !hasLevels.h5) {
        headings.filter(h => h.level === 'h6').forEach(h6 => {
            issues.push({
                type: 'missing_level',
                level: 'h6',
                index: h6.index,
